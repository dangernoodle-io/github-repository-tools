...
public class WorkflowExecutorTest
{
    ...
    private WorkflowExecutor createExecutor(Collection<Workflow> workflows)
    {
        return new WorkflowExecutor(workflows, Arrays.asList(mockPrePost));
    }
    ...
    {
        executor = createExecutor(Arrays.asList(mockGithubWorkflow));
    }
    ...
    private void whenExecuteWorkflow()
    {
        try
        {
            executor.execute(repository, Collections.emptyMap());
        }
        catch (Exception e)
        {
            exception = e;
